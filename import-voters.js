...
// Voter data from the HTML file
const voterData = `1	ALFIYA T	ANANTHU V N	/228	Sohanantham	F / 23	SEC043631150
...
// Parse and insert voters
const lines = voterData.trim().split('\n');
...
for (const line of lines) {
  ...
  if (parts.length >= 7) {
    ...
    const name = parts[1].replace(/^DELETED\s+/, '');
    const guardian_name = parts[2];
    const house_number = parts[3];
    const house_name = parts[4];
    const genderAge = parts[5].split(' / ');
    const gender = genderAge[0].trim() === 'M' ? 'Male' : genderAge[0].trim() === 'F' ? 'Female' : 'Other';
    const age = parseInt(genderAge[1]);
    const epic_no = parts[6];

    // Check if marked as deleted
    const isDeleted = parts[1].startsWith('DELETED');
...
console.log(`Imported ${count} voters from first batch`);
